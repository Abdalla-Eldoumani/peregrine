...
def _pg_chain(x, y, z):
    ...
    # arithmetic step.
    t = pg.axpby(x, y, a=A, b=B)
    t = pg.fma3(t, y, z)
    return pg.scaled_relu(t, scale=SCALE)
...
def _cpu_chain_series(sizes: list[int], reps: int, warmup: int) -> list[dict]:
    ...
    # NumPy chain are timed through _bench; the speedup is numpy_min / pg_min (the
    ...
    for elems in sizes:
        ...
        pg_t = _bench(lambda: _pg_chain(x, y, z), reps, warmup)
        ...
        speedup_floor = numpy_t["min_s"] / pg_t["min_s"]
        case = {
            "elems": elems,
            "shape": list(x.shape),
            "dtype": "float32",
            ...
            "speedup_median_readout": numpy_t["median_s"] / pg_t["median_s"],
            "statistic": "floor(min)",
            "floor_caveat": (
        ...
        series.append(case)
        print(
            f"elems={elems:>9d}  cpu  pg floor {pg_t['min_s']*1e3:8.2f} ms"
            f"  numpy floor {numpy_t['min_s']*1e3:8.2f} ms"
            f"  speedup {speedup_floor:5.2f}x (floor)"
            f"  cv {pg_t['cv']*100:5.1f}%/{numpy_t['cv']*100:5.1f}% (readout)"
        )
    return series
...
def _gpu_chain_series(sizes: list[int], reps: int, warmup: int) -> list[dict]:
    ...
    # it and NO pg._core._cuda_time_fused reference is ever reached on the OFF
    ...
    for elems in sizes:
        ...
        dt = pg.axpby(xd, yd, a=A, b=B)
        dt = pg.fma3(dt, yd, zd)
        dout = pg.scaled_relu(dt, scale=SCALE)
        got = pg.from_device(dout)
        ...
        warm_ms = pg._core._cuda_time_fused(xd, yd, zd, reps, warmup)
        # Cold: a single event-timed chain before the clocks warm.
        cold_ms = pg._core._cuda_time_fused(xd, yd, zd, 1, 0)
